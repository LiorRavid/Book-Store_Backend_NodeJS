 ...
 router.get('/books/search/:id', async(req,res)=>{
    try{
        ...
        for(let i = 0; i < books.length; i++){
            ...
            if(books[i].name.toLowerCase().includes(searchValue.toLowerCase()))
                result.push(books[i]);
        }
        if( !result)
 ...
 */
 router.post('/book/add',adminAuth,async (req,res)=>{
    try{
...
/**
 * Add an array of books to the data base.
 */
router.post('/books/add',async (req,res)=>{
    try{
        const books = [];
        for(let i=0;i<req.body.length;i++){
            const book = new Book(req.body[i]);
            books.push(book)
        }
        console.log(books);
        ...
        console.log('Error inserting books:', e);
        res.status(500).send(e);
    }
});
